...
df <- data.frame(Scenario = rep(c("Primary Analysis (n = 60)","Increased Sample Size (n = 120)", "Low Inter-Survey Correlation (n = 60)", "High Time Effect (n = 60)"), each = 4), Parameter = rep(c("$\\gamma_0$", "$\\beta_1$", "$\\beta_2$", "$\\beta_3$"),4),dat)
row.names(df) <- NULL
